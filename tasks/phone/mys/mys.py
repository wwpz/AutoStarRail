    ...
    def signin(self):
        for i in range(2):
            ...
            if auto.find_element("./res/phone/mys/mys_home.png", is_global=True):
                TimeUtils.wait_(2)
                for j in range(6):
                    if auto.find_element("./res/phone/mys/star_rail_text1.png",
                                         is_global=True) or auto.find_element(
                        "./res/phone/mys/star_rail_text2.png", is_global=True):
                        TimeUtils.wait_(5)
                        if auto.find_element("./res/phone/mys/mys_comment_icon.png", is_global=True):
                            TimeUtils.wait_(5)
                            if auto.find_element("./res/phone/mys/mys_like_icon.png", is_global=True):
                                TimeUtils.wait_(3)
                                auto.mouse_right_click(self.center_x, self.center_y)
                                TimeUtils.wait_(3)
                if auto.find_element("./res/phone/mys/star_rail_signin_icon.png", is_global=True):
                    TimeUtils.wait_(3)
                    if auto.find_element("./res/phone/mys/star_rail_signin.png", is_global=True):
                        TimeUtils.wait_(3)